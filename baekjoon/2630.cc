...
matrix_t copyMatrix(const matrix_t &matrix, std::pair<size_t, size_t> row_range,
                    std::pair<size_t, size_t> col_range) {
...
pair<int, int> split_matrix(const matrix_t &matrix) {
  ...
  return split_matrix(top_left) + split_matrix(top_right) +
         split_matrix(bottom_left) + split_matrix(bottom_right);
}
